...
matrix= pd.read_excel (r'C:\Users\bcerda\Documents\UiPath\p001_asignación_de_roles\out/optimization_base.xlsx',"base").fillna(0)
...
roles_weight=[2,2,1,1,1,1,1,1,2]
...
for i in range(len(matrix)):
    ...
    p= int(matrix.iloc[i]["pre_allocated"])
    P[(w,r,d)]= p
#print(P)
...
m.objective= xsum(X[w,r,d]*R[r] for w in range(len(workers)) for r in range(len(roles)) for d in range(len(days)))\
                ...
                + xsum(X[w,r,d]*R[r] for w in range(1) for r in range(1) for d in range(len(days)))*100 \
                - xsum(X[w,r,d] for w in range(len(workers)) for r in range(len(roles)) for d in range(len(days)-2,len(days)))*100 
...
for w in range(len(workers)):
    m.add_constr(MIN[w]<=xsum(X[w,r,d] + P[(w,r,d)] for r in range(len(roles)) for d in range(len(days))))

#MIP Constraint MAX relation
    
for w in range(len(workers)):
    m.add_constr(xsum(X[w,r,d] + P[(w,r,d)] for r in range(len(roles)) for d in range(len(days)))<=MAX[w])
...
for w in range(len(workers)):
        for r in range(len(roles)):
            if r==0 and w==0:
                m.add_constr(xsum(X[w,r,d] + P[(w,r,d)]  for d in range(len(days))) <= xsum(X[w,r,d]\
...
#m.emphasis = 1
#m.max_gap = 0.05
status = m.optimize(max_seconds=300,max_seconds_same_incumbent=10)
